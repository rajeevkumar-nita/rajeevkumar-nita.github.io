...
/**
 * Renders the Home/Hero section of the portfolio.
 */
const Home = () => {
  useEffect(() => {
    AOS.init({ duration: 1000, once: true });
  ...
  return (
    <section
      id="home"
      // This gradient background works for both light and dark mode
      className="bg-gradient-to-r from-[#6A11CB] to-[#2575FC] text-white text-center py-20 scroll-mt-32 relative"
    >
      <div className="max-w-6xl mx-auto px-4">
        {/* Profile Image: Added dark mode ring offset color */}
        <img
          src="/assets/Rajeev_kumar.jpg"
          alt="Rajeev Kumar"
          className="w-32 h-32 rounded-full mx-auto mb-6 border-4 border-white transform transition-all duration-500 ease-in-out
                   hover:scale-110 hover:rotate-3 hover:brightness-110 hover:shadow-2xl
                   hover:ring-4 hover:ring-yellow-300 hover:ring-offset-2 hover:ring-offset-blue-600 dark:hover:ring-offset-slate-900"
        />
        ...
        <p
          ...
          data-aos-delay="100"
        >
          Associate Software Engineer @ <span className="text-yellow-300 font-semibold">GlobalLogic</span>
        </p>
        ...
        <div className="mt-10 flex flex-nowrap justify-center gap-3 sm:gap-5 overflow-x-auto">
          {socialLinks.map(({ href, icon, img, label }, idx) => (
            <a
              ...
              aria-label={label}
            >
              {/* Social Icon Circle: Added dark mode background and hover */}
              <div className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full bg-white dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 shadow-md hover:shadow-xl transition-transform duration-300 hover:scale-110">
                {icon ? (
                  // Icon: Added dark mode text color
                  <span className="text-xl text-gray-800 dark:text-slate-200">{icon}</span>
                ) : (
                  // Image (e.g., LeetCode, GFG): No change needed
                  <img src={img} alt={label} className="w-6 h-6" />
                )}
              </div>
            </a>
